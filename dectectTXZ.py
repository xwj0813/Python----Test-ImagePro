...
class TextDectTXZ (object):
    ...
    def transform(self):
        ...
        return resize_img
    def hyp_paramters(self):
        #计算经验参数
        h,w,c=np.shape(self.resize_img)
        #文字区域最小最大面积
        min_area,max_area=25*25,0.04*h*w
        #文字区块最小最大高度
        min_h,max_h=30,100
    ...
    def find_candidates(self):
        ...
        #二值化
        _threshold=cv2.threshold(gray,115,255,cv2.THRESH_BINARY)[1]
        #腐蚀
        kernel=np.ones((2,4),np.uint8)
        erosion=cv2.erode(_threshold,kernel,iterations=2)
        cv2.imwrite("F:/image/Word/thresh.jpg",_threshold)
        #去噪
        ...
        #根据经验参数获取区域
        candidate_boxes=[]
        min_area,max_area,min_h,max_h=self.hyp_paramters()
        for cnt in contours:
    ...
    def split_candidates(self,box):
        ...
        gray=cv2.cvtColor(self.resize_img,cv2.COLOR_BGR2GRAY)
        #二值化
        _threshold = cv2.threshold(gray, 110, 255, cv2.THRESH_BINARY)[1]
        # 腐蚀
        kernel = np.ones((2, 4), np.uint8)
        erosion = cv2.erode(_threshold, kernel, iterations=2)
        # 获取区块对应腐蚀图像
    ...
    def is_noisy(self,box):
        ...
        h, w, c = np.shape(self.resize_img)
        x1, y1, x2, y2 = box
        if 0.15*h<(y1 + y2) / 2 and (x1 + x2) / 2<0.28 * w and (y1 + y2) / 2 < 0.8* h :
            return True
        return False
